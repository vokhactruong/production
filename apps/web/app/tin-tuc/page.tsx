...
async function getArticles(page = 1) {
  try {
    const res = await fetch(`${API_URL}/articles/public?page=${page}&limit=12`, { next: { revalidate: 60 } });
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {articlesData.items.map((article) => (
          <article key={article.id} className="rounded-2xl border border-slate-200 bg-white overflow-hidden hover:shadow-md transition-shadow">
            ...
            <div className="p-5">
              {article.category && <span className="rounded-lg bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">{article.category.name}</span>}
