import { useEffect } from "react";
import { useNavigate, useParams } from "react-router-dom";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { ArrowLeft, AlertCircle } from "lucide-react";
import axios from "axios";
import { useQueryClient } from "@tanstack/react-query";
import { useClass } from "../../features/classes/hooks/use-class";
import { useCreateClass } from "../../features/classes/hooks/use-create-class";
import { useUpdateClass } from "../../features/classes/hooks/use-update-class";
import { classKeys } from "../../features/classes/hooks/query-keys";
import { useCourses } from "../../features/courses/hooks/use-courses";
import { useSubjects } from "../../features/subjects/hooks/use-subjects";
import { useEmployees } from "../../features/employees/hooks/use-employees";
import { useClassrooms } from "../../features/classrooms/hooks/use-classrooms";
import { useToast } from "../../components/Toast";
import { cn } from "../../utils";
import { CLASS_STATUS_CONFIG } from "./constants";

// ─── Schema ───────────────────────────────────────────────────────────────────

const schema = z
  .object({
    code: z.string().min(1, "Vui lòng nhập mã lớp học").max(50, "Mã không được quá 50 ký tự"),
    name: z.string().min(1, "Vui lòng nhập tên lớp học").max(200, "Tên không được quá 200 ký tự"),
    courseId: z.string().min(1, "Vui lòng chọn khóa học"),
    subjectId: z.string().min(1, "Vui lòng chọn môn học"),
    classroomId: z.string().min(1, "Vui lòng chọn phòng học"),
    employeeId: z.string().min(1, "Vui lòng chọn giáo viên"),
    status: z.enum(["PLANNING", "OPEN", "FULL", "COMPLETED", "CANCELLED"]),
    capacity: z.coerce
      .number({ invalid_type_error: "Vui lòng nhập sức chứa" })
      .int("Phải là số nguyên")
      .positive("Sức chứa phải lớn hơn 0"),
    sessionCount: z.coerce
      .number({ invalid_type_error: "Vui lòng nhập số buổi học" })
      .int("Phải là số nguyên")
      .positive("Số buổi học phải lớn hơn 0"),
    startDate: z.string().min(1, "Vui lòng chọn ngày bắt đầu"),
    endDate: z.string().min(1, "Vui lòng chọn ngày kết thúc"),
    description: z.string().max(1000).optional(),
  })
  .refine((data) => new Date(data.endDate) >= new Date(data.startDate), {
    message: "Ngày kết thúc phải sau hoặc bằng ngày bắt đầu",
    path: ["endDate"],
  });

type FormData = z.infer<typeof schema>;

// ─── Shared primitives ────────────────────────────────────────────────────────

function Section({
  title,
  description,
  children,
}: {
  title: string;
  description?: string;
  children: React.ReactNode;
}) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white">
      <div className="border-b border-slate-100 px-6 py-4">
        <h3 className="text-base font-semibold text-slate-900">{title}</h3>
        {description && <p className="mt-0.5 text-xs text-slate-500">{description}</p>}
      </div>
      <div className="p-6">{children}</div>
    </div>
  );
}

function Field({
  label,
  required,
  error,
  children,
}: {
  label: string;
  required?: boolean;
  error?: string;
  children: React.ReactNode;
}) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-sm font-medium text-slate-700">
        {label}
        {required && <span className="ml-0.5 text-red-500">*</span>}
      </label>
      {children}
      {error && <p className="text-xs text-red-600">{error}</p>}
    </div>
  );
}

const inputCls = (hasError?: boolean) =>
  cn(
    "h-10 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-900",
    "placeholder:text-slate-400 transition-colors",
    "focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent",
    "disabled:bg-slate-50 disabled:text-slate-500 disabled:cursor-not-allowed",
    hasError && "border-red-500 focus:ring-red-500"
  );

const textareaCls = (hasError?: boolean) =>
  cn(
    "w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900",
    "placeholder:text-slate-400 resize-none transition-colors",
    "focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent",
    hasError && "border-red-500 focus:ring-red-500"
  );

// ─── Page ─────────────────────────────────────────────────────────────────────

export default function ClassForm() {
  const { id } = useParams<{ id?: string }>();
  const isEdit = Boolean(id);
  const navigate = useNavigate();
  const { emitToast } = useToast();
  const qc = useQueryClient();

  const {
    register,
    handleSubmit,
    reset,
    formState: { errors },
  } = useForm<FormData>({
    resolver: zodResolver(schema),
    defaultValues: { status: "PLANNING", sessionCount: 15 },
  });

  const {
    data: classItem,
    isLoading: isFetching,
    isError: isFetchError,
    refetch,
  } = useClass(isEdit ? id : undefined);

  const { data: coursesData } = useCourses({ page: 1 });
  const { data: subjectsData } = useSubjects({ page: 1 });
  const { data: employeesData } = useEmployees({ page: 1 });
  const { data: classroomsData } = useClassrooms({ page: 1 });

  const createClass = useCreateClass();
  const updateClass = useUpdateClass(id ?? "");
  const isPending = createClass.isPending || updateClass.isPending;

  useEffect(() => {
    if (classItem) {
      reset({
        code: classItem.code,
        name: classItem.name,
        courseId: classItem.courseId,
        subjectId: classItem.subjectId,
        classroomId: classItem.classroomId,
        employeeId: classItem.employeeId,
        status: classItem.status,
        capacity: classItem.capacity,
        sessionCount: classItem.sessionCount,
        startDate: classItem.startDate.slice(0, 10),
        endDate: classItem.endDate.slice(0, 10),
        description: classItem.description ?? "",
      });
    }
  }, [classItem, reset]);

  if (isEdit && isFetching) {
    return (
      <div className="flex items-center justify-center py-24 text-slate-400">
        <div className="flex flex-col items-center gap-3">
          <div className="h-6 w-6 animate-spin rounded-full border-2 border-slate-300 border-t-blue-500" />
          <span className="text-sm">Đang tải dữ liệu...</span>
        </div>
      </div>
    );
  }

  if (isEdit && isFetchError) {
    return (
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 flex items-center gap-4">
          <button
            type="button"
            onClick={() => navigate("/classes")}
            className="flex items-center justify-center rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-colors"
            aria-label="Quay lại"
          >
            <ArrowLeft className="h-4 w-4" />
          </button>
          <h2 className="text-2xl font-bold text-slate-900">Chỉnh sửa lớp học</h2>
        </div>
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-20 shadow-sm">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-red-50">
            <AlertCircle className="h-7 w-7 text-red-400" />
          </div>
          <p className="mt-4 text-sm font-semibold text-slate-700">
            Không thể tải thông tin lớp học
          </p>
          <p className="mt-1 text-xs text-slate-400">Vui lòng thử lại hoặc quay về danh sách</p>
          <div className="mt-5 flex gap-3">
            <button
              type="button"
              onClick={() => navigate("/classes")}
              className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors"
            >
              Quay lại danh sách
            </button>
            <button
              type="button"
              onClick={() => refetch()}
              className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 transition-colors"
            >
              Thử lại
            </button>
          </div>
        </div>
      </div>
    );
  }

  const activeCourses = coursesData?.items.filter((c) => c.status === "ACTIVE") ?? [];
  const activeSubjects = subjectsData?.items.filter((s) => s.status === "ACTIVE") ?? [];
  const activeEmployees = employeesData?.items.filter((e) => e.status === "ACTIVE") ?? [];
  const activeClassrooms = classroomsData?.items.filter((c) => c.isActive) ?? [];

  return (
    <div className="mx-auto max-w-2xl">
      {/* Header */}
      <div className="mb-6 flex items-center gap-4">
        <button
          type="button"
          onClick={() => navigate("/classes")}
          className="flex items-center justify-center rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-colors"
          aria-label="Quay lại"
        >
          <ArrowLeft className="h-4 w-4" />
        </button>
        <div>
          <h2 className="text-2xl font-bold text-slate-900">
            {isEdit ? "Chỉnh sửa lớp học" : "Thêm lớp học"}
          </h2>
          <p className="mt-0.5 text-sm text-slate-500">
            {isEdit ? "Cập nhật thông tin lớp học" : "Tạo lớp học mới"}
          </p>
        </div>
      </div>

      <form
        onSubmit={handleSubmit(async (data) => {
          const payload = {
            code: data.code,
            name: data.name,
            courseId: data.courseId,
            subjectId: data.subjectId,
            classroomId: data.classroomId,
            employeeId: data.employeeId,
            status: data.status,
            capacity: data.capacity,
            sessionCount: data.sessionCount,
            startDate: data.startDate,
            endDate: data.endDate,
            description: data.description?.trim() || undefined,
          };
          try {
            if (isEdit) {
              const { code: _c, ...updatePayload } = payload;
              await updateClass.mutateAsync(updatePayload);
            } else {
              await createClass.mutateAsync(payload);
            }
            await qc.refetchQueries({ queryKey: classKeys.lists(), type: "all" });
            emitToast(isEdit ? "Cập nhật lớp học thành công" : "Tạo lớp học thành công", "success");
            navigate("/classes");
          } catch (err) {
            const msg = axios.isAxiosError(err)
              ? ((err.response?.data as { message?: string })?.message ?? "Có lỗi xảy ra")
              : "Có lỗi xảy ra";
            emitToast(msg, "error");
          }
        })}
        noValidate
      >
        <div className="flex flex-col gap-5">
          {/* ── Basic Information ────────────────────────────────────────────── */}
          <Section title="Thông tin lớp học" description="Mã và tên lớp học">
            <div className="grid grid-cols-1 gap-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Field label="Mã lớp học" required error={errors.code?.message}>
                  <input
                    {...register("code")}
                    placeholder="VD: ENG-A01"
                    autoComplete="off"
                    disabled={isEdit}
                    className={inputCls(!!errors.code)}
                  />
                  {isEdit && (
                    <p className="text-xs text-slate-400">
                      Mã lớp học không thể thay đổi sau khi tạo
                    </p>
                  )}
                </Field>

                <Field label="Tên lớp học" required error={errors.name?.message}>
                  <input
                    {...register("name")}
                    placeholder="VD: English Basic A01"
                    autoComplete="off"
                    className={inputCls(!!errors.name)}
                  />
                </Field>
              </div>

              <Field label="Mô tả" error={errors.description?.message}>
                <textarea
                  {...register("description")}
                  rows={3}
                  placeholder="Mô tả ngắn về lớp học..."
                  className={textareaCls(!!errors.description)}
                />
              </Field>
            </div>
          </Section>

          {/* ── Assignment ────────────────────────────────────────────────────── */}
          <Section title="Phân công" description="Khóa học, môn học, giáo viên và phòng học">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Khóa học" required error={errors.courseId?.message}>
                <select
                  {...register("courseId")}
                  className={cn(inputCls(!!errors.courseId), "cursor-pointer")}
                >
                  <option value="">Chọn khóa học...</option>
                  {activeCourses.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.name}
                    </option>
                  ))}
                </select>
              </Field>

              <Field label="Môn học" required error={errors.subjectId?.message}>
                <select
                  {...register("subjectId")}
                  className={cn(inputCls(!!errors.subjectId), "cursor-pointer")}
                >
                  <option value="">Chọn môn học...</option>
                  {activeSubjects.map((s) => (
                    <option key={s.id} value={s.id}>
                      {s.name}
                    </option>
                  ))}
                </select>
              </Field>

              <Field label="Giáo viên" required error={errors.employeeId?.message}>
                <select
                  {...register("employeeId")}
                  className={cn(inputCls(!!errors.employeeId), "cursor-pointer")}
                >
                  <option value="">Chọn giáo viên...</option>
                  {activeEmployees.map((e) => (
                    <option key={e.id} value={e.id}>
                      {e.firstName} {e.lastName} ({e.code})
                    </option>
                  ))}
                </select>
              </Field>

              <Field label="Phòng học" required error={errors.classroomId?.message}>
                <select
                  {...register("classroomId")}
                  className={cn(inputCls(!!errors.classroomId), "cursor-pointer")}
                >
                  <option value="">Chọn phòng học...</option>
                  {activeClassrooms.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.name}
                    </option>
                  ))}
                </select>
              </Field>
            </div>
          </Section>

          {/* ── Schedule & Capacity ──────────────────────────────────────────── */}
          <Section
            title="Sức chứa & Thời gian học"
            description="Sức chứa, số buổi và thời gian học"
          >
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Sức chứa" required error={errors.capacity?.message}>
                <input
                  {...register("capacity")}
                  type="number"
                  min={1}
                  placeholder="VD: 20"
                  className={inputCls(!!errors.capacity)}
                />
              </Field>

              <Field label="Số buổi học / chu kỳ" required error={errors.sessionCount?.message}>
                <input
                  {...register("sessionCount")}
                  type="number"
                  min={1}
                  placeholder="VD: 15"
                  className={inputCls(!!errors.sessionCount)}
                />
              </Field>

              <Field label="Ngày bắt đầu" required error={errors.startDate?.message}>
                <input
                  type="date"
                  {...register("startDate")}
                  className={inputCls(!!errors.startDate)}
                />
              </Field>

              <Field label="Ngày kết thúc" required error={errors.endDate?.message}>
                <input
                  type="date"
                  {...register("endDate")}
                  className={inputCls(!!errors.endDate)}
                />
              </Field>
            </div>
          </Section>

          {/* ── Status ──────────────────────────────────────────────────────── */}
          <Section title="Trạng thái">
            <Field label="Trạng thái" required error={errors.status?.message}>
              <select
                {...register("status")}
                className={cn(inputCls(!!errors.status), "cursor-pointer")}
              >
                {Object.entries(CLASS_STATUS_CONFIG).map(([value, cfg]) => (
                  <option key={value} value={value}>
                    {cfg.label}
                  </option>
                ))}
              </select>
            </Field>
          </Section>

          {/* ── Footer ──────────────────────────────────────────────────────── */}
          <div className="flex justify-end gap-3 pb-2">
            <button
              type="button"
              onClick={() => navigate("/classes")}
              className="rounded-xl border border-slate-300 px-5 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors"
            >
              Huỷ
            </button>
            <button
              type="submit"
              disabled={isPending}
              className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isPending
                ? isEdit
                  ? "Đang lưu..."
                  : "Đang tạo..."
                : isEdit
                  ? "Lưu thay đổi"
                  : "Tạo lớp học"}
            </button>
          </div>
        </div>
      </form>
    </div>
  );
}
