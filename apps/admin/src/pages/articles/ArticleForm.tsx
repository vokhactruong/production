...
export default function ArticleForm() {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err) => {
      emitToast(axios.isAxiosError(err) ? (err.response?.data as { message?: string })?.message ?? "Có lỗi" : "Có lỗi", "error");
    },
  });
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">{isEdit ? "Chỉnh sửa bài viết" : "Tạo bài viết mới"}</h2>
        </div>
        <button onClick={() => navigate("/articles")} className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">
          ← Quay lại
        </button>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          <div className="rounded-2xl bg-white border border-slate-200 p-6 flex flex-col gap-4">
            <div>
              ...
              <input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Nhập tiêu đề bài viết..." className="h-10 w-full rounded-xl border border-slate-300 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              {slugPreview && <p className="mt-1 text-xs text-slate-500">Slug: <code>{slugPreview}</code></p>}
            </div>
            ...
            <div>
              ...
              <textarea value={excerpt} onChange={(e) => setExcerpt(e.target.value)} maxLength={500} rows={3} placeholder="Tóm tắt ngắn về bài viết..." className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>

            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">Nội dung *</label>
              <RichEditor value={content} onChange={setContent} placeholder="Bắt đầu viết nội dung..." />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-2xl bg-white border border-slate-200 p-6 flex flex-col gap-4">
            ...
            <div>
              ...
              <select value={status} onChange={(e) => setStatus(e.target.value as typeof status)} className="h-10 w-full rounded-xl border border-slate-300 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div className="flex flex-col gap-2">
              <button type="button" onClick={() => saveMutation.mutate("DRAFT")} disabled={saveMutation.isPending || isUploading} className="h-10 w-full rounded-xl border border-slate-300 text-sm font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-50">
                {saveMutation.isPending ? "Đang lưu..." : "Lưu nháp"}
              </button>
              <button type="button" onClick={() => saveMutation.mutate(status)} disabled={saveMutation.isPending || isUploading} className="h-10 w-full rounded-xl bg-slate-800 text-sm font-semibold text-white hover:bg-slate-900 disabled:opacity-50">
                Lưu
              </button>
              {isEdit && (
                <button type="button" onClick={() => navigate(`/articles/${id}/preview`)} className="h-10 w-full rounded-xl border border-slate-300 text-sm font-medium text-slate-600 hover:bg-slate-50">
                  Xem trước
                </button>
              )}
            </div>
          ...
          <div className="rounded-2xl bg-white border border-slate-200 p-6 flex flex-col gap-4">
            ...
            <div>
              ...
              <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="h-10 w-full rounded-xl border border-slate-300 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <label className={cn(
                  ...
                  {isUploading ? (
                    ...
                    <>
                      ...
                      <span className="text-xs text-slate-400">JPG, PNG, WebP, GIF · Tối đa 5MB</span>
                    </>
                  )}
                </label>
              )}
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-700">Tags <span className="text-slate-400">({tags.length}/10)</span></label>
              <input value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={addTag} placeholder="Nhập tag rồi nhấn Enter..." className="h-10 w-full rounded-xl border border-slate-300 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              {tags.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {tags.map((tag) => (
                    <span key={tag} className="flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-0.5 text-xs text-slate-700">
                      {tag}
                      <button type="button" onClick={() => setTags(tags.filter((t) => t !== tag))} className="text-slate-400 hover:text-slate-600">×</button>
                    </span>
                  ))}
                </div>
              )}
